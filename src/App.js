...
class DataForm extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      success: false,
      message: "",
      passedAmount: qs.parse(window.location.search, {
  ...
  handleSubmit(event) {
    ...
    fetch(
      "https://beta.soluspay.net/api/v1/payment/sahil" +
        `?id=${this.state.id}&token=${this.state.token}&to="http://localhost:7327"`,
      {
        method: "POST",
        redirect: "follow",
        headers: {
      ...
      .then((data) => {
        ...
        this.setState({
          success: true,
          message: data.ebs_response.responseMessage,
          approval: data.ebs_response.responseMessage,
          error: false,
        });
        ...
        if (error.status >= 400 || error.status >= 500) {
          error.json().then((body) => {
            ...
            if (error.status > 500) {
              ...
              this.setState({
                message: body.details.responseMessage,
                error: true,
                username: null,
              });
            } else {
        ...
        <Box my={4}>
          ...
          <FormGroup>
            <form onSubmit={this.handleSubmit}>
              ...
              />
              <InputLabel htmlFor="pan">
                Enter your PAN (16 or 19 digits)
              </InputLabel>
          ...
          {this.state.error && (
            <div>
              ...
              <p onChange={this.handleChange}>
                There is an error: <b>{this.state.message}</b>
              </p>
              <Dialog
          ...
          {this.state.success && (
            <div>
              {/* <Redirect to="/success"/> */}
              <p onChange={this.handleChange}>
                Successful response <b>{this.state.message}</b>
              </p>
              <Dialog
                open={this.state.open}
                aria-labelledby="alert-dialog-title"
